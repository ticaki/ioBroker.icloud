            ...
            if (this.authStore.sessionToken) {
                try {
                    ...
                    if (validateResponse.status === 200) {
                        ...
                        try { this.accountInfo = await validateResponse.json() as any; } catch (_) { /* ignore */ }
                        this.authStore.saveSession(this.options.username!);
                ...
                if (this.authStore.processAuthSecrets(authResponse, this.options.username!)) {
                    ...
                    try {
                        ...
                        if (setupResp.status === 200) {
                            ...
                            accountLoginOk = true;
                            try { this.accountInfo = await setupResp.json() as any; } catch (_) { /* ignore */ }
                        } else {
    ...
    async refreshWebservices(): Promise<boolean> {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                try { this.accountInfo = await response.json() as any; } catch (_) { /* ignore */ }
                this._serviceCache = {};
                return true;
            }
            return false;
