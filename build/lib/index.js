...
class iCloudService extends import_events.default {
  ...
  */
  ...
  constructor(options) {
    ...
    if (!this.options.dataDirectory) this.options.dataDirectory = import_path.default.join(import_os.default.homedir(), ".icloud");
  ...
  async authenticate(username, password) {
    ...
    if (typeof username !== "string") throw new TypeError("authenticate(username?: string, password?: string): 'username' was " + (username || JSON.stringify(username)).toString());
    ...
    if (typeof password !== "string") throw new TypeError("authenticate(username?: string, password?: string): 'password' was " + (password || JSON.stringify(password)).toString());
      ...
      if (authResponse.status == 200) {
        ...
        if (this.authStore.processAuthSecrets(authResponse, this.options.username)) {
          ...
          try {
            ...
            this._log(LogLevel.Debug, "[auth] POST", import_consts.SETUP_ENDPOINT, "(accountLogin after 409 \u2014 triggers MFA push and may complete auth)");
            ...
            this._log(LogLevel.Debug, "[auth] accountLogin (post-409) failed:", pushTriggerErr.toString());
          }
          if (accountLoginOk) {
        ...
        if (authResponse.status == 503)
          throw new Error("RATE_LIMITED: Apple hat den Login vor\xFCbergehend gesperrt (HTTP 503). Bitte 30\u201360 Minuten warten und dann erneut versuchen.");
  ...
  async provideMfaCode(code) {
    if (typeof code !== "string") throw new TypeError("provideMfaCode(code: string): 'code' was " + code.toString());
    ...
    const authResponse = await this.fetch(
      import_consts.AUTH_ENDPOINT + "verify/trusteddevice/securitycode",
  ...
  async _getTrustToken() {
    ...
    const authResponse = await this.fetch(
      import_consts.AUTH_ENDPOINT + "2sv/trust",
  ...
  async _getiCloudCookies() {
    ...
    try {
      ...
      const response = await this.fetch(import_consts.SETUP_ENDPOINT, { headers: import_consts.DEFAULT_HEADERS, method: "POST", body: JSON.stringify(data) });
  ...
  getService(service) {
    ...
    if (service === "photos")
      this._serviceCache[service] = new this.serviceConstructors[service](this, (_c = webservices.ckdatabasews) == null ? void 0 : _c.url);
    if (!this._serviceCache[service])
      this._serviceCache[service] = new this.serviceConstructors[service](this, (_d = webservices[service]) == null ? void 0 : _d.url);
